...
const ScheduleItem: ReactComponent<PropsType> = props => {
	...
	const AirStatus = useMemo(() => {
		...
		if (
			dayjs().isBefore(AirStartTime) &&
			NextAirsData[0] &&
			AirStartTime === NextAirsData[0].s * 1000
		) {
			airStatus = 'next';
		}
	...
	const AirPhrase = useMemo(() => {
		...
		const IsAirsCollision = PreviousAir
			? (PreviousAir.s + PreviousAir.d) * 1000 === AirStartTime
			: false;
	...
	const IsNeedToShowAirYear =
		dayjs(AirEndTime).year() !== CurrentYear || dayjs(AirStartTime).year() !== CurrentYear;
	...
	return (
		<li className={styles.scheduleItem} data-status={AirStatus} data-theme={Theme}>
			...
			<div className={styles.scheduleItem__data}>
				<div className={styles.scheduleItem__startTime}>
					{formatAirTime(dayjs(AirStartTime).toDate(), IsNeedToShowAirYear)}
				</div>

				<div className={styles.scheduleItem__duration}>
					<span
						title={`Время окончания: ${formatAirTime(dayjs(AirEndTime).toDate(), IsNeedToShowAirYear)}`}>
